...
const ContactMails = () => {
    ...
    const fetchMessages = async () => {
        ...
        try {
            ...
            console.log('Contact API response:', data); // debug
            // Server returns { success: true, contacts: [...] }
            if (data.success && Array.isArray(data.contacts)) {
                setMessages(data.contacts);
            } else if (data.success && Array.isArray(data.messages)) {
                // Fallback if server property name changes
            ...
            {!loading && error && (
                <div className="flex-1 flex items-center justify-center">
                    <div className="text-center p-10">
                        ...
                        <h3 className="text-xl font-bold mb-2 text-red-400">Error loading messages</h3>
                        <p className="text-[var(--text-dim)] max-w-sm mx-auto text-sm mb-4">{error}</p>
                        <button onClick={fetchMessages} className="px-4 py-2 bg-[var(--primary)] text-white rounded-xl text-sm">
                            Retry
                        </button>
                    </div>
                </div>
